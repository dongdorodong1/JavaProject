...
/**
 * 회원 고객에게 추천상품을 제공하는 클래스입니다. <br\>캘린더 클래스를 이용하여 이번달과 전달의 월을 구하고 <br\> HashMap안에
 * 월에 해당하는 상품을 key로 하고, 상품의 갯수를 누적하는 방식으로 value로 하였습니다.<br\> 맵을 정렬하기위해
 ...
 *
 */
public class MemberRecommendProduct { // TODO동환

	public static void recommend(Member member) {
		...
		while (loop) {
			...
			if (sel.equals("1")) {
				// 1번메뉴가 장바구니로
				MemberMenuService.option(member, Data.getProduct(Data.getProductNo(oderList.get(0).getKey())));
			} else if (sel.equals("2")) {
				// 2번메뉴가 장바구니로
				MemberMenuService.option(member, Data.getProduct(Data.getProductNo(oderList.get(1).getKey())));
			} else if (sel.equals("3")) {
				// 3번메뉴가 장바구니로
				MemberMenuService.option(member, Data.getProduct(Data.getProductNo(oderList.get(2).getKey())));
			} else if (sel.equals("4")) {
				// 4번메뉴가 장바구니로
				MemberMenuService.option(member, Data.getProduct(Data.getProductNo(oderList.get(3).getKey())));
			} else if (sel.equals("5")) {
				// 5번메뉴가 장바구니로
				MemberMenuService.option(member, Data.getProduct(Data.getProductNo(BeforeOderList.get(0).getKey())));
			} else if (sel.equals("6")) {
				// 6번메뉴가 장바구니로
				MemberMenuService.option(member, Data.getProduct(Data.getProductNo(BeforeOderList.get(1).getKey())));
			} else if (sel.equals("7")) {
				// 7번메뉴가 장바구니로
				MemberMenuService.option(member, Data.getProduct(Data.getProductNo(BeforeOderList.get(2).getKey())));
			} else if (sel.equals("8")) {
				// 8번메뉴가 장바구니로
				MemberMenuService.option(member, Data.getProduct(Data.getProductNo(BeforeOderList.get(3).getKey())));
			} else {
				loop = false;
			}
		}
